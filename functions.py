...
def thermal_energy(stamps, kernel, label, aperature):
    """
    ...
    """ Returns the thermal energy in ergs
    Inputs:comov = comoving distance, z = redshift,   y = summed value of every pixel in stacked plot, theta = pixel size in arcsec

    """
        
    therm = 2.9 * (comovs/(1+z))**2 * (y2*(theta/60)**2)/(10**(-6))
    
    #therme = np.mean(enr)
    #errors = np.std(enr)/np.sqrt(len(enr))
    return therm
